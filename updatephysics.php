...
$sql = "SELECT * FROM physics ORDER BY bookname ASC";
...
if(mysqli_num_rows($result) > 0){
    ...
    if($decision == "Issue"){
        ...
        $sql = "SELECT * FROM record WHERE book_id = '$rphy_id' AND user_id = '$user_id'";
        ...
        //run query
        $sql = "UPDATE physics SET availablebook = availablebook - 1 WHERE phy_id = '$phy_id'";
        $results = mysqli_query($link, $sql);
        if(!$results){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
        ...
        $sql = "UPDATE booklist SET availablebooks = availablebooks - 1 WHERE book_id = '$rphy_id'";
        $results = mysqli_query($link, $sql);
        if(!$results){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
        ...
        $sql = "INSERT INTO record (user_id, book_id, bookname, authorname) VALUES ('$user_id', '$rphy_id', '$bookname', '$authorname')";
        $tresults = mysqli_query($link, $sql);
        if(!$tresults){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
    ...
    if($decision == "Return"){
        ...
        $sql = "DELETE FROM record WHERE book_id = '$rphy_id'";
        $tresults = mysqli_query($link, $sql);
        if(!$tresults){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
