...
$sql = "SELECT * FROM maths ORDER BY bookname ASC";
...
if(mysqli_num_rows($result) > 0){
    ...
    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
        $divId = $divId + 1;
    if(empty($_POST["mathscheck$divId"])){
        continue;
    }
        
    //get physics book id
    $maths_id = $row["maths_id"];
    $rmaths_id = "m$maths_id";
    $decision = $_POST["mathscheck$divId"];
    ...
    if($decision == "Issue"){
        ...
        $sql = "SELECT * FROM record WHERE book_id = '$rmaths_id' AND user_id = '$user_id'";
        ...
        //run query
        $sql = "UPDATE maths SET availablebook = availablebook - 1 WHERE maths_id = '$maths_id'";
        $results = mysqli_query($link, $sql);
        if(!$results){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
        ...
        $sql = "UPDATE booklist SET availablebooks = availablebooks - 1 WHERE book_id = '$rmaths_id'";
        $results = mysqli_query($link, $sql);
        if(!$results){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
        ...
        $sql = "INSERT INTO record (user_id, book_id, bookname, authorname) VALUES ('$user_id', '$rmaths_id', '$bookname', '$authorname')";
        $tresults = mysqli_query($link, $sql);
        if(!$tresults){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
    ...
    if($decision == "Return"){
        ...
        $sql = "DELETE FROM record WHERE book_id = '$rmaths_id'";
        $tresults = mysqli_query($link, $sql);
        if(!$tresults){
            echo "<div class='alert alert-danger'>Error running in query.Please try again later!</div>";
            exit;
